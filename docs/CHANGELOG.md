...
- **Music modes and DIY scenes are no longer offered as effects.** A music mode makes the device
  ...
  confirmed, and were silently forgotten at the next poll. Govee's saved snapshots go with them,
  for the same reason — and on this account they turned out to be the only ones of the three
  that any device actually had. They are hidden, not removed: an automation that already names one
  keeps working.
